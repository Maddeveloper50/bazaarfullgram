/*
 * This is the source code of Supergram for Android v. 1.3.x.
...
public class ContactsController {
    ...
    public void checkAppAccount() {
        ...
        Account[] accounts;
        try {
            accounts = am.getAccountsByType("org.Supergram.account");
        ...
        accounts = am.getAccountsByType("com.baranak.tsupergrap");
        ...
        if (recreateAccount) {
            ...
            if (UserConfig.isClientActivated()) {
                try {
                    currentAccount = new Account("" + UserConfig.getClientUserId(), "com.baranak.tsupergrap");
    ...
    public void deleteAllAppAccounts() {
        try {
            AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
            Account[] accounts = am.getAccountsByType("com.baranak.tsupergrap");
            for (int a = 0; a < accounts.length; a++) {
                am.removeAccount(accounts[a], null, null);
            }
        } catch (Exception e) {
        ...
        Utilities.globalQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                /*if (schedule) {
                    try {
                        AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                        Account[] accounts = am.getAccountsByType("org.Supergram.account");
                        boolean recreateAccount = false;
                        if (UserConfig.isClientActivated()) {
                            if (accounts.length != 1) {
                                FileLog.e("detected account deletion!");
                                currentAccount = new Account(UserConfig.getCurrentUser().phone, "org.Supergram.account");
                                am.addAccountExplicitly(currentAccount, "", null);
                                AndroidUtilities.runOnUIThread(new Runnable() {
                                    @Override
                                    public void run() {
...
//        query.add(builder.build());
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.com.baranak.tsupergrap.android.profile");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        builder.withValue(ContactsContract.Data.DATA2, "Supergram Profile");
